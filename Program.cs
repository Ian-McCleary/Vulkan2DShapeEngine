...
unsafe class HelloTriangleApplication
{
    ...
    private string windowName = "Vulkan C#";
    ...
    private void FramebufferResizeCallback(Vector2D<int> obj)
    {
        ...
        camera!.Resize(0, 0, (uint)obj.X, (uint)obj.Y);
        cameraController!.Resize(obj);
    }
    ...
    private void createVertexData(){
        ...
        for (int i = 1; i < 10000; i++){
            ...
            var xs = rand.Next(-100, 100);
            var ys = rand.Next(-100, 100);

            var len = 0.1f;
            var x = (float)xs/5;
            var y = (float)ys/10;

            vertices.Add(new Vertex { pos = new Vector2D<float>(x, y), color = new Vector3D<float>(1.0f, 0.0f, 0.0f) });
            vertices.Add(new Vertex { pos = new Vector2D<float>(x + len, y), color = new Vector3D<float>(0.0f, 1.0f, 0.0f) });
            vertices.Add(new Vertex { pos = new Vector2D<float>(x + len, y + len), color = new Vector3D<float>(0.0f, 0.0f, 1.0f) });
            vertices.Add(new Vertex { pos = new Vector2D<float>(x, y + len), color = new Vector3D<float>(1.0f, 1.0f, 1.0f) });

            ushort[] idx = {
                (ushort)(0 + (i * 4)), (ushort)(1 + (i * 4)), (ushort)(2 + (i * 4)), (ushort)(2 + (i * 4)), (ushort)(3 + (i * 4)), (ushort)(0 + (i * 4))
            };
    ...
    private void setupCamera() {
        camera = new OrthographicCamera(Vector3.Zero, 4f, 0, 0, window!.FramebufferSize);
        cameraController = new(camera, (IWindow)window);
        FramebufferResizeCallback(window.FramebufferSize);
    ...
    {
        var vertShaderCode = File.ReadAllBytes("C:/CamRepos/VulcanC#/shaders/vert.spv");
        var fragShaderCode = File.ReadAllBytes("C:/CamRepos/VulcanC#/shaders/frag.spv");
    ...
    private void CreateVertexBuffer()
    {
        ...
        ulong bufferSize = (ulong)(Unsafe.SizeOf<Vertex>() * vArray.Length);
        ...
        vArray.AsSpan().CopyTo(new Span<Vertex>(data, vArray.Length));
    ...
    {
        ushort[] iArray = indices.ToArray();
        ulong bufferSize = (ulong)(Unsafe.SizeOf<ushort>() * iArray.Length);
        ...
        void* data;
        vk!.MapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
        iArray.AsSpan().CopyTo(new Span<ushort>(data, iArray.Length));
        vk!.UnmapMemory(device, stagingBufferMemory);
    ...
    private void CreateCommandBuffers()
    {
        ...
        for (int i = 0; i < commandBuffers.Length; i++)
        {
            ...
            vk!.CmdDrawIndexed(commandBuffers[i], (uint)indices.Count, 1, 0, 0, 0);
    ...
    private Matrix4X4<float> ConvertSilkMatrixToSystemNumeric(Matrix4x4 m ){
        Matrix4X4<float> conversionMatrix = new Matrix4X4<float>(
    ...
    private void UpdateUniformBuffer(uint currentImage)
    {
        ...
        UniformBufferObject ubo = new()
        {
            ...
            view = ConvertSilkMatrixToSystemNumeric(camera!.GetViewMatrix()),
            proj = ConvertSilkMatrixToSystemNumeric(camera!.GetProjectionMatrix())
        };
        ubo.proj.M22 *= -1;
